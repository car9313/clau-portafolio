...
export function CardMarginBorders({
    ...
    >
      ...
      >
        <MovingBorder duration={duration} rx="30%" ry="30%">
          <div
            className={cn(
              "h-20 w-20 opacity-[0.8] bg-[radial-gradient(#818cf8_40%,transparent_60%)]",
              borderClassName
            )}
          />
        </MovingBorder>
...
export const MovingBorder = ({
  ...
  type PathLike = SVGPathElement | (SVGRectElement & {
    getTotalLength?: () => number
    getPointAtLength?: (n: number) => { x: number; y: number }
  })

  const pathRef = useRef<PathLike | null>(null)
